 ...
 */
/*
 * copied from javax.swing.undo
 */
package fr.inra.mig_bibliome.alvisae.client.Edit.undo;
 ...
 * @version %I%, %G%
 */
public class UndoManager extends CompoundEdit implements EditHappenedEventHandler {
     ...
     */
    public UndoManager(AnnotatedText annotatedText) {
        super();
        indexOfNextAdd = 0;
        limit = 100;
    ...
    public synchronized void discardAllEdits() {
        ...
        while (cursor.hasNext()) {
            UndoableEdit e = (UndoableEdit) cursor.next();
            e.die();
        }
...
//          System.out.println("new index " + indexOfNextAdd);
    ...
    public void onEditHappened(EditHappenedEvent event) {
        if (event instanceof UndoableEditHappenedEvent) {
            if (event.getEdit().getAnnotatedTextHandler().getAnnotatedText().equals(getAnnotatedText())) {
                addEdit(event.getEdit());
            }
        }
    }
    
    public synchronized String getWaterMark() {
        String waterMark = "";
        if (inProgress) {
            UndoableEdit edit = editToBeUndone();
            if (edit != null) {
                waterMark = edit.toString();
            }
        }
        return waterMark;
    }
